...
class Intensity:
    ...
    def __init__(self, expected_n: int, factor_mask):
        f"""Creates a expected_n intensity function over the FactorMask.
...
def get_unit_intensity(expected_n: int, dim: int):
    f"""Creates a expected_n intensity function over the unit dim hypercube.
    
    Args:
        {re.search('expected_n.+', Intensity.__init__.__doc__).group()}
...
def get_constrained_intensity(expected_n: int, ratio_estimator, x0: Tensor, threshold: float, factor_mask=None, samples: int = 10000):
    f"""Creates a expected_n intensity function constrained to ratio estimates over the treshold. 
    ...
    # TODO consider splitting the creation of the constrained mask (idk maybe not)
...
def construct_intervals(x, y):
    ...
    if len(upcrossings) - len(downcrossings) == 1:
        ...
        raise ValueError("It should be impossible to have two more crossings of one type, than the other.")
...
def get_factor_mask_from_intervals(intervals):
    ...
    if intervals.ndim == 2:
        ...
        raise ValueError("Intervals must be a list of intervals or a list of a list of intervals.")
