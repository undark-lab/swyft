...
class Mask1d:
    ...
    def __init__(self, intervals):
        ...
        Args:
            intervals (list of tuples of floats): (lower, upper) bounds on regions which yield one upon evaluation.
                intervals with very small length are ignored.
        """
        ...
        for x, y in measurable_intervals:
            assert increasing(x, y), "One of the intervals was decreasing. They need to be (lower bound, upper bound)."
...
class Intensity:
    ...
    def __init__(self, expected_n, mask, area=None):
        """
        Args:
            expected_n (int): expected target number of samples
            mask (FactorMask): a d-dimensional mask function
            area (float): (optional) area of positively masked region 
        """
    ...
    def __call__(self, z):
        return self.mask(z) / self.area * self.expected_n
    
    def __repr__(self):
        return f"{self.__class__.__name__}({self.expected_n}, {repr(self.mask)})"
...
def construct_intervals(x, y):
    ...
    """  # TODO a y which is entirely above zero will return the whole interval
    ...
    intervals = []
    for i in range(len(upcrossings)):
        intervals.append([x[upcrossings[i]], x[downcrossings[i]]])
