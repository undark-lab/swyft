...
class Network(nn.Module):
    def __init__(self, x_dim, z_dim, n_hidden, xz_init=None):
        ...
        self.x_dim = x_dim
        self.z_dim = z_dim
        self.fc1 = nn.ModuleList([nn.Linear(x_dim + 1, 200) for i in range(z_dim)])
        self.fc2 = nn.ModuleList([nn.Linear(200 + 1, n_hidden) for i in range(z_dim)])
...
plt.axvline(0.1)
plt.axvline(0.9)
plt.savefig("figs/testrun_05.png")
