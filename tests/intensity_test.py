...
import torch
import numpy as np
from swyft.intensity import construct_intervals, get_factor_mask_from_intervals, Mask1d, FactorMask, Intensity
...
class TestFactorMask:
    ...
    def test_get_factor_mask_from_intervals_2dim(self, xmax, area):
        ...
        factor_mask = get_factor_mask_from_intervals(intervals)
        assert np.isclose(round(area**2, 2), round(factor_mask.area(), 2), rtol=.001)


class TestConstructIntervals:
    xmaxs = [4*np.pi, 4*np.pi, 1.0, 1.0]
    fns = [np.sin, np.cos, lambda x: (x-0.5)**2-0.1, lambda x: -(x-0.5)**2+0.1]
    targets = [
        [[0.0, 3.140650081536511], [6.282556925810732, 9.424463770084952]],
        [[1.5696966593994006, 4.711603503673621], [7.853510347947842, 10.995417192222062]],
